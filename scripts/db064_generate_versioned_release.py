...
final catalogue, and DB060 is rerun before an artifact can be approved.
"""
...
def quality_report_markdown(manifest: dict[str, Any]) -> str:
    quality = manifest["quality"]
    exclusions = manifest["excluded_records"]
    return f"""# DB063 - Final Release Data Quality Check

**Result:** APPROVED DATASET ARTIFACT

**Dataset:** `{manifest['dataset']['file']}`
...
The combined release check passed for all {manifest['dataset']['record_count']:,}
...
def prepare_release(
    *,
    ...
    try:
        ...
        manifest = {
            "schema": "food-remedy-release-manifest-v1",
            ...
            "evidence": {
                ...
                "final_quality_report": "DB063-final-data-quality-check.md",
                "checksums": "SHA256SUMS",
            },
        }
        write_json(stage / "validation_manifest.json", manifest)
        (stage / "DB063-final-data-quality-check.md").write_text(
            quality_report_markdown(manifest), encoding="utf-8"
        )
        ...
        checksum_files = [
            ...
            "category_audit.json",
            "exclusions.json",
            "DB063-final-data-quality-check.md",
        ]
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(
        f"{manifest['release_status']}: {manifest['dataset']['record_count']} records; "
        f"sha256={manifest['dataset']['sha256']}; output={output_dir}"
    )
    return 0
