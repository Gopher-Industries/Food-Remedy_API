...
/**
 * Canonical profile-to-product evidence mapping shared by safety-sensitive
 * product flows. `positive-only` restrictions can identify a conflict but an
 * absent ingredient cannot prove a product safe.
 */
export const RESTRICTION_RULES: Readonly<Record<string, RestrictionRule>> = {
  ...
  Soy: { resolution: "declaration", aliases: ["soy", "soya", "soybean", "soybeans", "tofu", "edamame", "miso", "tempeh", "大豆"] },
  ...
  Gluten: { resolution: "declaration", aliases: ["gluten", "wheat", "barley", "rye", "oat", "oats", "triticale", "spelt", "semolina", "couscous", "malt", "contains cereals containing gluten", "gluten containing cereals", "wheaten", "wheat gluten", "weizenmehl", "小麦", "oat bran", "rolled barley", "avoine"] },
  Lactose: { resolution: "positive-only", aliases: ["lactose", "milk", "dairy", "casein", "whey", "butter", "cream", "cheese", "yoghurt", "yogurt", "milk powder", "milk solids", "lait", "magermilchpulver", "magemilchpulver", "butterfat", "lactic culture", "edamer", "kase"] },
...
export function findRestrictionRule(value: string): RestrictionRule | undefined {
  ...
  return Object.entries(RESTRICTION_RULES).find(([label]) => label.toLowerCase() === normalized)?.[1];
}
