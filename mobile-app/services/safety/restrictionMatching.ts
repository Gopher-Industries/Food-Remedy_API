...
function isSeafoodTerm(value: string): boolean {
  return Array.from(SEAFOOD_TERMS).some(
    (term) => value === term || value.includes(`${term} `) || value.includes(` ${term}`)
  );
}

/** Conservative match used by every backend path that makes a safety decision. */
export function restrictionsMatch(restriction: string, productTerm: string): boolean {
  const expected = normaliseRestriction(restriction);
  const actual = normaliseRestriction(productTerm);

  if (!expected || !actual) return false;
  if (isSeafoodTerm(expected) && isSeafoodTerm(actual)) return true;
