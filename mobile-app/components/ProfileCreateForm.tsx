...
const ProfileCreateForm = () => {
  ...
  const handleSave = async () => {
    ...
    if (!name.trim()) {
      addNotification("Please enter your name.", "e");
      return false;
    }
    ...
    try {
      ...
      if (user?.uid) {
        try {
          await upsertUserProfile(user.uid, created.profileId, created);
        } catch (e) {
      ...
      addNotification("Profile created successfully.", "s");
      return true;
    } catch (err: any) {
      console.error(err);
  ...
  const handleSaveAndNext = async () => {
    ...
    if (saved) {
      router.replace("/(app)/demographics" as any);
    }
  };
      ...
      <View className="flex-1 w-[90%] self-center">
        ...
        >
          <Tt className="text-xl font-interBold text-center my-4">
            Create Your Nutritional Profile
          </Tt>
          ...
          >
            ...
            <Tt className="text-xl font-interMedium text-left">
              {avatarUri ? "Change Picture" : "Upload Picture"}
            </Tt>
          </Pressable>
