...
export default function CheckoutPage() {
  const { listId, shoppedBarcodes } = useLocalSearchParams<{
    listId: string;
    shoppedBarcodes: string;
  }>();
  ...
  const getHighlight = (item: any) => {
    ...
    const hasGluten =
      hasIngredientText &&
      glutenKeywords.some((keyword: string) =>
        ingredientTextLower.includes(keyword)
      );

    const hasGlutenFreeLabel = labels.some(
      (label: string) =>
        label.toLowerCase().includes("gluten") &&
        label.toLowerCase().includes("free")
    );

    const isGlutenFree = hasGlutenFreeLabel || (hasIngredientText && !hasGluten);
    const isOrganic = labels.some((label: string) =>
      label.toLowerCase().includes("organic")
    );

    if (highProtein && proteinKnown) return "💪 Rich in Protein";
    ...
    if (
      ...
      !highSatFat &&
      !highSalt &&
      (healthyNutriscore || (lowSugar && lowSatFat && lowSalt))
    ) {
    ...
    if (isGlutenFree && hasIngredientText) return "🌾 Gluten Free";
    if (isOrganic) return "🌱 Organic";
    if (highFiber && fiberKnown) return "🌾 High Fiber";
    if (lowSugar && sugarKnown) return "🍬 Low Sugar";
    if (highSugar && sugarKnown) return "🍬 High Sugar";

    const nutrition = product.enrichment?.nutrition;

    if (
      nutrition &&
      Array.isArray(nutrition.reasons) &&
      ...
      if (
        reasonText.includes("low") &&
        (reasonText.includes("sugar") ||
          reasonText.includes("fat") ||
          reasonText.includes("salt"))
      ) {
        return "❤️ Heart Healthy";
      }

      if (reasonText.includes("carb") || reasonText.includes("carbohydrate")) {
        return "🍞 Low Carb";
      }
  ...
  const renderProductCard = (item: any) => {
    ...
    return (
      <View
        key={item.barcode}
        className={`flex-row items-center py-3.5 px-4 rounded-3xl shadow-sm ${
          darkMode ? "bg-hsl20 shadow-none" : "bg-white"
        }`}
      >
        <View className="flex-1">
          <Tt
            className={`font-interSemiBold text-sm ${
              darkMode ? "text-white" : "text-hsl30"
            }`}
            numberOfLines={2}
          >
            {item.productName}
  ...
  if (!ready || loading) {
    return (
      <Screen className={`p-safe ${darkMode ? "bg-hsl15" : "bg-white"}`}>
        ...
        <View className="flex-1 items-center justify-center">
          ...
          <Tt className={`mt-3 text-sm ${darkMode ? "text-hsl70" : "text-hsl50"}`}>
            Loading checkout…
          </Tt>
        </View>
      </Screen>
  ...
  return (
    <Screen className={`p-safe ${darkMode ? "bg-hsl15" : "bg-white"}`}>
      ...
      >
        ...
        >
          <Tt className="text-white font-interSemiBold text-sm">Go Home</Tt>
        </Pressable>
      </View>
    </Screen>
  );
