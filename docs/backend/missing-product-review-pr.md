...
The separate existing barcode regression suite could not run because this Python
environment lacks pytest. Workflow tests exercise the real barcode and required-field
...
PR creation remains outstanding: this environment has neither the GitHub connector
nor the `gh` CLI. This file is the prepared PR description, not a published PR.
